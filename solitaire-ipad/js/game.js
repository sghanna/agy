...
class SolitaireGame {
  ...
  render() {
    ...
    for (let c = 0; c < 7; c++) {
      ...
      let currentTopCalc = [];
      let selectedFirstCalc = null;
      let selectedLastCalc = null;
      let lastSelectedCardEl = null;

      cards.forEach((card, index) => {
        const cardEl = window.SolitaireDeck.createCardElement(card);
        const topVal = currentTopCalc.length ? `calc(${currentTopCalc.join(' + ')})` : '0px';
        cardEl.style.top = topVal;
        ...
        if (isSelected) {
          cardEl.classList.add('selected-stack-card');
          if (selectedFirstCalc === null) selectedFirstCalc = topVal;
          selectedLastCalc = topVal;
          lastSelectedCardEl = cardEl;
        }

        // Use CSS variables for vertical exposure
        if (card.faceUp) {
          currentTopCalc.push('var(--up-step, 32px)');
        } else {
          currentTopCalc.push('var(--down-step, 12px)');
        }
      ...
      if (selectedFirstCalc !== null && selectedLastCalc !== null) {
        const halo = document.createElement('div');
        halo.className = 'stack-selection-halo';
        halo.id = 'active-selection-halo';
        ...
        if (lastSelectedCardEl && lastSelectedCardEl.offsetHeight > 0) {
          ...
          cardHeight = colEl.clientWidth * 1.5;
        }

        halo.style.top = `calc((${selectedFirstCalc}) - 5px)`;
        halo.style.left = '-1px';
        halo.style.width = 'calc(100% + 2px)';
        halo.style.height = `calc((${selectedLastCalc}) - (${selectedFirstCalc}) + ${cardHeight}px + 2px)`;
